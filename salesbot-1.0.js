...
Fails miserably. 
Lucky if it picks up more than one fruit.
...
function getShortestRoute(board, x, y)
{
    ...
    while(time + (9 * 60) > Date.now()) // repeat until we've spent 9 seconds here (1 second buffer)
    {
        x = original_x;
        y = original_y;
        var scoring = [];
        ...
        for(var i=0;i<20;i++) // simulate offspring
        {
            ...
            score -= offspring[i].length / max_moves;
